...
def run_etl(
    ...
    return APIResponse(data=ETLService(db).run(source_path=request.source_path))
...
def run_etl_from_s3(
    ...
    return APIResponse(data=ETLService(db).run_from_s3(s3_key=request.s3_key, s3_prefix=request.s3_prefix))
